...
fn main() {
    let (path, lines) = MINE.split_once("\n\n").unwrap();
    let lines = lines.split("\n").collect::<Vec<&str>>();
    //  println!("{:?}", part1(path, &lines));
    println!("{:?}", part2_v2(path, &lines));
}

fn part1(path: &str, input: &[&str]) -> u32 {
...
fn part2(path: &str, input: &[&str]) -> u32 {
    let g = parse_input(input);
...
fn part2_v2(path: &str, input: &[&str]) -> u32 {
    let g = parse_input(input);
    ...
    let mut loops: Vec<_> = (0..cur.len()).map(|v| HashSet::new()).collect();
    for j in 0..1000 {
...
const MINE: &str = "
